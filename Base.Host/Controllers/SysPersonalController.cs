...
namespace Base.Host.Controllers
{
    ...
    public class SysPersonalController : BaseController
    {
        ...
        [HttpPatch, HttpPut]
        [Route("Password")]
        public async Task<BaseMessage> UpdatePasswordAsync([FromBody] ResetPasswordVo password)
        {
            var msg = new BaseMessage();
        ...
        {
            var msg = new BaseMessage();
            msg.Data = await _service.GetWxAccessTokenAsync(LoginUser.Id.TryGuid());
        ...
        {
            var msg = new BaseMessage();
            msg.Data = await _service.GetWxgzhSubscribeUserAsync(LoginUser.Id.TryGuid());
            return msg.Success("成功");
        }
