...
namespace Base.Host.Controllers
{
    ...
    public class SysArticlesController : BaseController
    {
        ...
        public async Task<BaseMessage> AddAsync([FromBody]SysArticleForm form)
        {
            ...
            msg.ErrType = await _articleService.AddAsync(LoginUser, LoginUser.TenantId.TryGuid(), form);
            ...
            [FromForm]IFormCollection form)
        {
            ...
            if (form.Files.Count > 0)
            {
                ...
                var callbacks = await _articleService.UploadCoverAsync(LoginUser.TenantId.TryGuid(), id, file.FileName, file.OpenReadStream());
            ...
            [FromForm]IFormCollection form)
        {
            ...
            if (form.Files.Count > 0)
            {
                var file = form.Files[0];
                if (id.Equals(Guid.Empty)) id = Guid.NewGuid(); // 实现先传图再创建对象
                var callbacks = await _articleService.UploadImageAsync(LoginUser.TenantId.TryGuid(), id, file.FileName, file.OpenReadStream());
